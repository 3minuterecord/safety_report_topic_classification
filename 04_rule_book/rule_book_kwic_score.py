...
def main():
        # Load test data
        sample_ref = '220908222547'
        try:
                out_df = pd.read_csv(f"13_test_samples/{sample_ref}_100_sample_scores.csv", dtype=str)  
                ...
                out_df.to_csv(f'06_scored_samples/{sample_ref}_{classified_count}_scored_dataset_{goods_per}-{fairs_per}-{badas_per}.csv', index=False)
                print('done \n')
        except:
