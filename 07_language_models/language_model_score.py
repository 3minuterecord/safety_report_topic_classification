...
def main():
        ...
        sample_size = 100
        try:
                out_df = pd.read_csv(f"01_data/fabricated/{sample_ref}.csv", dtype=str)
                ...
                out_df.to_csv(f'06_scored_samples/{sample_ref}_scored_dataset_{goods_per}-{fairs_per}-{badas_per}.csv', index=False)
                print('done \n')
        except:
